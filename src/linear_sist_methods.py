...
def eliminacion_gaussiana(A: np.ndarray | list[list[float | int]]) -> np.ndarray:
    ...
    assert A.shape[0] == A.shape[1] - \
        1, "La matriz A debe ser de tamaño n-by-(n+1)."
    n = A.shape[0]
    ...
    for i in range(0, n - 1):  # loop por columna
        ...
        for j in range(i + 1, n):
            m = A[j, i] / A[i, i]
            cont_mult_div += 1
            longitud_fila = len(A[j, i:])
            A[j, i:] = A[j, i:] - m * A[i, i:]
            cont_mult_div += longitud_fila
            cont_sum_res += longitud_fila
    ...
    for i in range(n - 2, -1, -1):
        ...
        cont_sum_res += 1
        cont_mult_div += 1
    print(f"Total sumas/restas: {cont_sum_res}")
    print(f"Total mult/div: {cont_mult_div}")
    return solucion
